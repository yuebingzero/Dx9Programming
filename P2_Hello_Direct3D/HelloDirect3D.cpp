...
void HelloD3DApp::onResetDevice()
{
	...
	// This code sets texture filters, which helps to smooth
	// out distortions when you scale a texture.
	...
	HR(gd3dDevice->SetSamplerState(990, D3DSAMP_MIPFILTER, D3DTEXF_LINEAR));
...
void HelloD3DApp::updateBullets(float dt)
{
	...
	while (i != mBulletList.end())
	{
		...
		i->life += dt;
		// If the bullet has lived for two seconds, kill it.
...
{
	// Set a texture coordinate scaling transform. Here we scale
	// the texture coordinates by 10 in each dimension. This
	// tiles the texture ten times over the sprite surface.
	...
	HR(mSprite->Flush());
	// Restore default texture coordinate scaling transform. Mirror
	// texture coordinates with respect to y-axis so the texture is
	// mapped correctly onto the sprite.
...
void HelloD3DApp::drawShip()
{
	...
	// Draw the ship.
	HR(mSprite->Draw(mShipTex, 0, &mShipCenter, 0,
...
void HelloD3DApp::drawBullets()
{
	...
	while (i != mBulletList.end())
	{
		...
		// Add it to the batch.
		HR(mSprite->Draw(mBulletTex, 0, &mBulletCenter, 0,
			D3DCOLOR_XRGB(255, 255, 255)));
		++i;
	}
